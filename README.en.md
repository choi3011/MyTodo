...
- [ ] Pause polling when window loses focus, not only on minimize.
- [ ] Replace REST polling with the Firestore Listen channel (gRPC) for true push.
- [ ] CI: build on every PR, auto-publish MSI/DMG on release tags.
- [ ] iOS client via the Compose Multiplatform iOS target.
